...
standard_freq = 2500.0  # standard tone at 2300 Hz  was 2300 in exp2.1

# low =  np.arange(1750.0, 2050.0+30.0, 30.0)# mean 1900   
# mid = np.arange(2350.0, 2650.0+30.0, 30.0) # mean 2500
# high = np.arange(2950.0, 3250.0+30.0, 30.0)  # mean 3100

low =  np.arange(1600.0, 2200.0+60.0, 60.0)# mean 1900  
mid = np.arange(2200.0, 2800.0+60.0, 60.0) # mean 2500
high = np.arange(2800.0, 3400.0+60.0, 60.0)  # mean 3100
...
def print_mean_summary():
    print('\n')
    print('low')
    print(sorted(create_precursor_freqs(low)), np.mean(create_precursor_freqs(low)))

    print('mid')
    print(sorted(create_precursor_freqs(mid)), np.mean(create_precursor_freqs(mid)))

    print('high')
    print(sorted(create_precursor_freqs(high)), np.mean(create_precursor_freqs(high)))


    print('\n#################\n')

    print('lowmid')
    print(create_precursor_freqs(low,mid), np.mean(create_precursor_freqs(low,mid)))

    print('midlow')
    print(create_precursor_freqs(mid,low), np.mean(create_precursor_freqs(mid,low)))



    print('highmid')
    print(create_precursor_freqs(high,mid), np.mean(create_precursor_freqs(high,mid)))

    print('midhigh')
    print(create_precursor_freqs(mid,high), np.mean(create_precursor_freqs(mid,high)))
...
with open("exp3_block2_stimlist.csv",'w') as resultFile:
    wr = csv.writer(resultFile)
    wr.writerows(all_stims)


# Create stim list for block1
with open("exp3_block1_stimlist.csv",'w') as resultFile:
...
#### Block 1
with open("exp3_practice1_stimlist.csv",'w') as resultFile:
    resultFile.write('stim_fname\n')
...
lowmid = create_precursors([create_precursor_freqs(low, mid)], sr, tone_dur)

midlow = create_precursors([create_precursor_freqs(mid, low)], sr, tone_dur)

highmid = create_precursors([create_precursor_freqs(high, mid)], sr, tone_dur)

midhigh = create_precursors([create_precursor_freqs(mid, high)], sr, tone_dur)


practice_precursors = [('lowmid', lowmid), ('midlow', midlow), ('highmid', lowmid), ('midhigh', midlow)]*int(len(practice_stim_files)/4)
...
with open("exp3_practice2_stimlist.csv",'w') as resultFile:
    wr = csv.writer(resultFile)
